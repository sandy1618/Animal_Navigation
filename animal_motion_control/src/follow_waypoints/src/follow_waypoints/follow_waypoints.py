...
def convert_PoseWithCovArray_to_PoseArray(waypoints):
    ...
    return poses
def convert_PoseStamped_to_PoseWithCovarianceStamped(PoseStamped):
    "Used to convert poseArray to PosewithcovArray for converting goal pos. to cov. array"
    pose = PoseWithCovarianceStamped()
    pose.header.frame_id = rospy.get_param("~goal_frame_id",'map')
    ...
    def execute(self, userdata):
        ...
        while (not self.path_ready and not self.start_journey_bool):
            try:
                ...
                pose_stamped = rospy.wait_for_message(topic, PoseStamped, timeout=1)
                pose = convert_PoseStamped_to_PoseWithCovarianceStamped(pose_stamped)
